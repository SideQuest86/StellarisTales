...
export function createDeckMotion(root, reduced) {
  let selected=0, frame=0, last=0, elapsed=0;
  let states=[];
  function setup(){ states=[...root.children].map(()=>({x:0,y:0,r:0,vx:0,vy:0,vr:0})); }
  function tick(now){
    frame=0;
    if(document.hidden) return;
    const dt=Math.min((now-last)/1000||1/60,1/30); last=now; elapsed+=dt;
    let moving=false;
    [...root.children].forEach((el,i)=>{
      const s=states[i]; if(!s)return;
      const length=states.length;
      const distance=((i-selected+Math.floor(length/2)+length)%length)-Math.floor(length/2);
      const target={x:distance*65,y:Math.abs(distance)*19+(distance===0?-36:10),r:distance*-5};
      for(const k of ['x','y','r']){
        const v='v'+k;
        if(reduced()){s[k]=target[k];s[v]=0;}
        else {s[v]+=(target[k]-s[k])*125*dt;s[v]*=Math.exp(-19*dt);s[k]+=s[v]*dt;}
        if(Math.abs(target[k]-s[k])>.02||Math.abs(s[v])>.02)moving=true;
      }
  ...
  return {select(i){selected=i;wake();},reset(){setup();wake();},refresh:wake};
}
